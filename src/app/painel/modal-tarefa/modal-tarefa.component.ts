...
export class ModalTarefaComponent implements OnInit {
  ...
  public modo: ModoFormulario = ModoFormulario.CADASTRO;
  public configuracoes: any;
  public carregando: boolean = false;
  public tarefa?: Tarefa;
  private erro?: string;

  @ViewChild('dataPrevistaConclusao') elDataPrevistaConclusao: any;

  constructor(
    private fb: FormBuilder,
    private modalTarefaService: ModalTarefaService,
    private tarefaApiService: TarefasApiService
  ) {
  ...
  ngOnInit(): void {
    this.modalTarefaService.escutarEvento((config: ModalTarefaConfig) => {
      ...
      if (this.estaEmModoEdicao()) {
        this.elDataPrevistaConclusao.nativeElement.type = 'date';
        this.form.controls.nome.setValue(this.tarefa?.nome);
        this.form.controls.dataPrevistaConclusao.setValue(
  ...
  getErros() {
    ...
    if (this.campoJaFoiManipulado('nome')) {
      ...
      if (this.form.controls.nome.errors?.minlength) {
        erros.push('Nome deve ter no minimo 4 caracteres');
      }
    }

    if (this.campoJaFoiManipulado('dataPrevistaConclusao')
      && this.form.controls.dataPrevistaConclusao.errors?.required) {
  ...
  campoJaFoiManipulado(campo: string) {
    return this.form.controls[campo].dirty || this.form.controls[campo].touched;
  }
  ...
  async submit() {
    ...
    try {
      ...
      this.erro = e.error && e.error.message
        ? e.error.message
        : 'Ocorreu algum erro ao salvar a tarefa, tente novamente mais tarde';
    }
